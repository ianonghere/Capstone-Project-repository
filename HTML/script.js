...
function loadLogsFromBackend() {
  fetch("/api/v1/audit/")
    ...
    .then(logs => {
      ...
      // Filter out older duplicates (since logs list starts newest-first)
      const uniqueLogs = logs.filter(log => {
        const queryKey = log.query.trim();
      ...
      // Traverse in reverse (oldest to newest) to unshift newest to the top
      uniqueLogs.reverse().forEach(log => {
        const mappedStatus = log.status === "discarded" ? "rejected" : log.status;
        const isEmail = log.channel === "email";
        
        const existingTicket = isEmail ? null : state.tickets.find(t => t.id === log.session_id || t.session_id === log.session_id);
        const existingEmail = isEmail ? state.emails.find(e => e.id === log.session_id || e.session_id === log.session_id) : null;

        if (existingTicket) {
          if (existingTicket.status !== mappedStatus) {
            existingTicket.status = mappedStatus;
            hasUpdates = true;
            if (state.selectedTicketId === log.session_id || state.selectedTicketId === existingTicket.id) {
        ...
        if (existingEmail) {
          ...
          if (existingEmail.read !== emailRead || existingEmail.archived !== emailArchived) {
            existingEmail.read = emailRead;
            existingEmail.archived = emailArchived;
            existingEmail.folder = emailArchived ? "archive" : "inbox";
            hasUpdates = true;
            if (emailArchived && state.selectedEmailId === log.session_id) {
              state.selectedEmailId = null;
            }
          }
        }

        if (isEmail && existingEmail) return;
        if (!isEmail && existingTicket) return;

        // Parse From email and From name
        let name = "Student";
        ...
        if (!isEmail) {
          ...
          state.tickets.unshift({
            ...
            channel: "portal",
            status: mappedStatus,
            priority: log.confidence && log.confidence < 75 ? "high" : "medium",
            intent: "Email Inquiry",
            category: "External Inquiry",
            body: log.query,
            aiDraft: log.generated_draft,
            confidence: log.confidence || 80,
            citations: [],
            flags: log.confidence && log.confidence < 75 ? ["Low confidence check required"] : [],
            receivedAt: log.created_at
          });
        } else {
          // Also add to Email Inbox
